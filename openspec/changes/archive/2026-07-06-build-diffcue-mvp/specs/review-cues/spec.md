...
A cue's `file` is the relative path from the opened folder root; `line` is the line
number on the indicated `side` (old or new) at the time the cue was created. The system
SHALL NOT auto-rewrite cue line numbers when the working tree changes; a cue whose
target line no longer exists SHALL be flagged as "stale" in the cue list and the marker
SHALL render in a warning style.
