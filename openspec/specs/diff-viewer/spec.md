...
Side-by-side and inline (unified) diff rendering with syntax highlighting (via `ImGuiColorTextEdit`), line-ending + encoding metadata per line, Prev/Next-change hunk navigation across the working set, and read-only text find (Ctrl+F).
