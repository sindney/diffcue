...
CMake build that produces a runnable `diffcue` executable on Win64, macOS, and Linux, statically linking ImGui + ImGuiColorTextEdit + GLFW3 + tinyfiledialogs from vendored sources. `git` is a runtime prerequisite (no libgit2).
